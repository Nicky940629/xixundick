    ...
    def generate_signals(self):
        ...
        for i in range(200, len(df)):  # 從第200天開始，確保SMA200計算完整
            ...
            if prev_high_above_sma200 and previous_high > 0:
                ...
                '''
                # 第二次進場條件：前高回調30%，有30%倉位時再投入30%資金（總共60%）
                elif entry_30_condition and prev_pos_30 > 0 and prev_pos_60 == 0:
                    investment_amount = prev_cash_balance * 0.3  # 使用剩餘現金的30%
                    shares_to_buy = investment_amount / current_price
                    
                    df.iloc[i, df.columns.get_loc('Signal')] = 1
                    df.iloc[i, df.columns.get_loc('Position_60')] = shares_to_buy
                    df.iloc[i, df.columns.get_loc('Total_Position')] = prev_pos_30 + shares_to_buy
                    df.iloc[i, df.columns.get_loc('Entry_Price_60')] = current_price
                    df.iloc[i, df.columns.get_loc('Cash_Balance')] = prev_cash_balance - investment_amount
                    df.iloc[i, df.columns.get_loc('Position_Value')] = position_value + investment_amount
                    
                    df.iloc[i, df.columns.get_loc('Trade_Reason')] = f'Entry 30% at 30% drawdown from previous high {previous_high:.2f}: Current {current_price:.2f}, Investment: ${investment_amount:.2f}'
                    '''
        self.signals = df
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(f"\n=== 📊 策略特色 ===")
        print("✅ 前高（含局部高點）在SMA200以上時才進場")
        print("✅ 從前高回調20%投入30%資金")
        print("✅ 從前高回調30%再投入30%資金（總共60%）")
        print("✅ 30%倉位跌20%止損，60%倉位跌10%止損")
        print("✅ 創新高時止贏")
        print("✅ 複利滾動計算，資金動態增長")
        print("✅ 局部高點也視為前高，增加交易機會") 